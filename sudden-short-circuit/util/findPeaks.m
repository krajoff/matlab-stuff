    ...
    for i = 1:length(data)-1
        ...
        if time(i)>opt.timeSpan(1) && time(i)<opt.timeSpan(2)
            ...
            if data(i)*data(i+1)<=0
                 % Разделение на верхние (1) и нижние (-1) нули
                if data(i)>0
                    result(end+1,:) = [i,1]; 
                elseif data(i)<0
                    result(end+1,:) = [i,-1]; 
...
function opt = getPeaks(nils,time,data,opt,name)
    ...
    % Сохранение пиков
    opt.peaks.(name).maxPeaks = array2table(maxPeaks, 'VariableNames', {'Time', 'Value'});
    opt.peaks.(name).minPeaks = array2table(minPeaks, 'VariableNames', {'Time', 'Value'});
end
